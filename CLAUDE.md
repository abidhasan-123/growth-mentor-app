...
A growth mentor app that anchors goal-setting to a 10-year vision and generates weekly scorecards scoring progress across health, education, and soft-skill goals.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "302162354+abidhasan-123@users.noreply.github.com"
  git config user.name "abidhasan-123"
  ```
